...
impl LinuxUsbGadgetManager {
    ...
    async fn ensure_configfs_mounted(&self) -> Result<(), HardwareError> {
        ...
        if !path.exists() {
            ...
            let output = Command::new("sudo")
                ...
                .output()
                .await
                .map_err(|e| HardwareError::SystemCommandFailed(format!("Failed to mount configfs: {}", e)))?;

            if !output.status.success() {
                let stderr = String::from_utf8_lossy(&output.stderr);
                return Err(HardwareError::GadgetConfigurationFailed(format!("Failed to mount configfs: {}", stderr)));
            }
        }
        Ok(())
    }
    ...
    async fn get_available_udc(&self) -> Result<String, HardwareError> {
        ...
        let mut entries = fs::read_dir(udc_path)
            .await
            .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to read UDC directory: {}", e)))?;

        while let Some(entry) = entries.next_entry().await
            .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to read UDC entry: {}", e)))? {
            if let Ok(name) = entry.file_name().into_string() {
                return Ok(name);
            }
        }

        Err(HardwareError::GadgetConfigurationFailed("No UDC available".to_string()))
    }

    async fn write_file(&self, path: &str, content: &str) -> Result<(), HardwareError> {
        fs::write(path, content)
            .await
            .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to write {}: {}", path, e)))?;
        Ok(())
    }
    ...
    async fn create_directory(&self, path: &str) -> Result<(), HardwareError> {
        fs::create_dir_all(path)
            .await
            .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to create directory {}: {}", path, e)))?;
        Ok(())
    }
}
...
impl UsbGadgetRepository for LinuxUsbGadgetManager {
    async fn create_gadget(&self, gadget: &UsbGadget) -> Result<(), HardwareError> {
        ...
        self.write_file(&format!("{}/idVendor", gadget_path), &format!("0x{:04x}", gadget.descriptor.vendor_id)).await?;
        self.write_file(&format!("{}/idProduct", gadget_path), &format!("0x{:04x}", gadget.descriptor.product_id)).await?;
        self.write_file(&format!("{}/bcdDevice", gadget_path), &format!("0x{:04x}", gadget.descriptor.device_version)).await?;
        self.write_file(&format!("{}/bcdUSB", gadget_path), &format!("0x{:04x}", gadget.descriptor.usb_version)).await?;
        ...
        self.write_file(&format!("{}/serialnumber", strings_path), &gadget.descriptor.serial_number).await?;
        self.write_file(&format!("{}/manufacturer", strings_path), &gadget.descriptor.manufacturer).await?;
        self.write_file(&format!("{}/product", strings_path), &gadget.descriptor.product).await?;
    ...
    async fn configure_gadget(&self, gadget: &UsbGadget) -> Result<(), HardwareError> {
        ...
        self.write_file(&format!("{}/MaxPower", config_path), "500").await?;
        ...
        self.write_file(&format!("{}/configuration", config_strings_path), "Nintendo Switch Pro Controller").await?;
        ...
        self.write_file(&format!("{}/protocol", function_path), "0").await?;
        self.write_file(&format!("{}/subclass", function_path), "0").await?;
        self.write_file(&format!("{}/report_length", function_path), "8").await?;
        ...
        fs::write(&report_desc_path, &report_desc)
            .await
            .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to write report descriptor: {}", e)))?;

        // Link function to configuration
        let symlink_path = format!("{}/hid.usb0", config_path);
        if !Path::new(&symlink_path).exists() {
            std::os::unix::fs::symlink(&function_path, &symlink_path)
                .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to create symlink: {}", e)))?;
        }
    ...
    async fn get_gadget_state(&self, gadget_id: &str) -> Result<UsbGadget, HardwareError> {
        ...
        if !path.exists() {
            return Err(HardwareError::GadgetConfigurationFailed(format!("Gadget {} not found", gadget_id)));
        }
    ...
    async fn remove_gadget(&self, gadget_id: &str) -> Result<(), HardwareError> {
        ...
        if Path::new(&symlink_path).exists() {
            fs::remove_file(&symlink_path).await
                .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to remove symlink: {}", e)))?;
        }
        ...
        for dir in dirs_to_remove {
            if Path::new(&dir).exists() {
                fs::remove_dir(&dir).await
                    .map_err(|e| HardwareError::FileOperationFailed(format!("Failed to remove {}: {}", dir, e)))?;
            }
        }
